...
```{r}
qplot(x = sweet_level, data = wdata)
```
...
```{r}
qplot(x = quality, data = wdata, binwidth = 1) +
...
ggplot(aes(x = density, y = alcohol), data = wdata) + 
  geom_point(col=ifelse(wdata$density>1.01, "red", "black"), alpha = 0.5, 
...
```{r}
qplot(x = quality, y = free.sulfur.dioxide,  data = wdata) + 
  geom_point(alpha = 0.5, size = 1, position = 'jitter')
...
```{r}
one <- qplot(x = quality, data = wdata, binwidth = 1) +
  facet_wrap(~alc_level) +
  aes(fill = alc_level) +
