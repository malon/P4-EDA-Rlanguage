...
Notes: 5 flavors of dannon yougurt. It is micro data since we have a line in the dataset for each of the purchases.
```{r}
...
#also by seeing how many distince prices are in the dataset, only 20
...
set.seed(4230)
# gettin a sample of 16 households, we are sampling from levels (of factors) because those are all the different housholds that I have
...
### Many Variables
Notes:
We should let the data speak to determine variables of interest. There´s a tool we can use to create a number of scatter plots automatically: a scatterplot matrix. In a scatterplot matrix there is a grid of scatter plots between any par of variables. Scatter plots are great but not necessarily suitable for any kind of variables like categorical ones.In these cases box plots or histograms which are better.
...
We want to see for each combination of gene and sample, the difference in gene expression and the sample from the base line. Comginations where a gene is over expressed in red and underexpressed in blue.
